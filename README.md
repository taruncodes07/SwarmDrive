...
The result is a system where cooperation emerges — vehicles slow to create zipper gaps, clear lanes for ambulances, and recover formation smoothly after disturbances — without any of these behaviours being hard-coded.
...
- **Scenario fidelity**: The Ambulance Yield scenario models real emergency-vehicle-approach dynamics (proximity range, closing speed, lane occupancy) with a 21-term reward function tuned to produce legally correct yielding behaviour.
...
- **Scenario 03 — Ambulance Yield**: An emergency vehicle approaches from behind at up to 26 m/s. Agents must detect the siren broadcast, assess lane occupancy, and execute a clean lane change before the ambulance closes to dangerous proximity.
